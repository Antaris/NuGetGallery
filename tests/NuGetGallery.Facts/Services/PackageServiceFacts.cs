    ...
    public class PackageServiceFacts
    {
        ...
        public class TheCreatePackageMethod
        {
            ...
            public void WillReadThePropertiesFromTheNuGetPackageWhenCreatingANewPackage()
            {
                ...
                Assert.Equal(
                    "theFirstDependency:[1.0.0, 2.0.0):net4|theSecondDependency:[1.0.0, 1.0.0]:net4|theThirdDependency:(, ):net4|theFourthDependency:[1.0.0, 1.0.0]:net35",
            ...
            public void WillStoreTheHashForTheCreatedPackage()
            {
                ...
                var packageBytes = nugetPackage.Object.GetStream().ReadAllBytes();
                var expectedHash = CryptographyService.GenerateHash(packageBytes, Constants.Sha512HashAlgorithmId);
            ...
            public void WillSaveThePackageFileAndSetThePackageFileSize()
            {
                ...
                var packageBytes = nugetPackage.Object.GetStream().ReadAllBytes();
                var packageHash = CryptographyService.GenerateHash(packageBytes, Constants.Sha512HashAlgorithmId);
                var packageStreamMetadata = new PackageStreamMetadata
                {
                    Hash = packageHash,
                    HashAlgorithm = Constants.Sha512HashAlgorithmId,
            ...
            [Fact]
            private void WillThrowIfThePackageDependencyIdIsLongerThan128()
            {
                var service = CreateService();
                var nugetPackage = CreateNuGetPackage(packageDependencyGroups: new[]
                    {
            ...
            private void WillSaveSupportedFrameworks()
            {
                ...
                Assert.Equal("net4", package.SupportedFrameworks.First().TargetFramework);
